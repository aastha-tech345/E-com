...
export function ShopProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      toast.error("Unable to save shop state in browser storage");
    }
  }, [state, hydrated]);
  ...
  const addToCart: ShopContextValue["addToCart"] = useCallback(
    (productId, quantity = 1, opts) => {
      if (opts?.product) productService.remember(opts.product);
      patch((s) => {
      ...
      syncBackendCart(cartService.addProduct(productId, quantity));
      toast.success("Added to cart");
    },
    [patch, syncBackendCart],
  );
