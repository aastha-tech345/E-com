...
//#region node_modules/.nitro/vite/services/ssr/assets/shop-fTXyFsSH.js
...
var API_BASE = {
	...
	"VITE_API_URL": "http://localhost:8009/api/v1"
}["VITE_API_URL"] || "http://localhost:8000/api/v1";
var assistantConversationId;
var SHOP_STATE_KEY = "shopnest-state-v1";
function getStoredAccessToken() {
	...
	try {
		const parsed = JSON.parse(shopState);
...
var productService = {
	...
	byIds(ids) {
		return ids.map((id) => this.byId(id)).filter((product) => Boolean(product));
	},
	all() {
		return products;
	},
...
var orderService = {
	async list() {
		...
		return (await response.json()).map(toOrder);
	},
	async byId(id) {
		if (!getStoredAccessToken()) return void 0;
	...
	async byStatus(status) {
		return (await this.list()).filter((order) => order.status === status);
	},
	async byCustomer(customerId) {
		return (await this.list()).filter((order) => order.customerId === customerId);
	}
};
var paymentService = { async createStripeCheckoutSession(payload) {
...
var chatbotService = { async reply(message) {
	try {
	...
	const matchedProducts = products.filter((product) => {
		...
		return lower.split(/\s+/).filter((part) => part.length > 2).some((part) => haystack.includes(part));
	}).slice(0, 3);
	return {
		id: crypto.randomUUID(),
		role: "assistant",
		text: matchedProducts.length > 0 ? "I found a few products that match what you're looking for." : "I can help you find products, compare prices, or look up product IDs.",
...
function toAssistantProductResult(product) {
...
function buildOrderTimeline(status, date) {
	return [
		{
			label: "Order placed",
			date,
...
function isCartLine(value) {
	return isRecord(value) && typeof value["productId"] === "string" && typeof value["quantity"] === "number" && Number.isFinite(value["quantity"]);
}
function isChatMessage(value) {
	return isRecord(value) && typeof value["id"] === "string" && (value["role"] === "user" || value["role"] === "assistant") && typeof value["text"] === "string";
}
function stringArray(value) {
	return Array.isArray(value) ? value.filter((item) => typeof item === "string") : [];
}
function isAuthTokens(value) {
	return isRecord(value) && typeof value["access_token"] === "string" && typeof value["refresh_token"] === "string";
}
function normalizeStoredState(value) {
	...
	return {
		...
		user: isRecord(value["user"]) ? value["user"] : EMPTY.user,
		admin: isRecord(value["admin"]) ? value["admin"] : EMPTY.admin,
		tokens: isAuthTokens(value["tokens"]) ? value["tokens"] : EMPTY.tokens,
		addresses: Array.isArray(value["addresses"]) ? value["addresses"] : EMPTY.addresses,
...
//#endregion
export { catalogService as a, orderService as c, productService as d, products as f, brands as i, paymentService as l, adminUsers as n, categories as o, useShop as p, authService as r, chatbotService as s, ShopProvider as t, productAttributes as u };
